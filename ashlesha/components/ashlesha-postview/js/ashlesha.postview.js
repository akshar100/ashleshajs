YUI().add('ashlesha-postview',function(Y) {
    ...
    Y.PostView = Y.Base.create("PostView", Y.AshleshaBaseView, [], {
        ...
        loadYoutubeVideos:function(){
            var model = this.get("model"),
                postText = model.get("posttext"),
                pattern = /https?:\/\/(?:[a-zA_Z]{2,3}.)?(?:youtube\.com\/watch\?)((?:[\w\d\-\_\=]+&amp;(?:amp;)?)*v(?:&lt;[A-Z]+&gt;)?=([0-9a-zA-Z\-\_]+))/i,
                matches = postText.match(pattern),
                youtube = this.get("template").one("#"+this.name+"-YoutubeEmbedd").getHTML(),
                c = this.get('container');
                
                if(matches && matches.length>=3){
                    console.log(matches);
                    youtube = Y.Lang.sub(youtube,{ 
                            ID:matches[2]
                        });
                    setTimeout(function() { c.one(".youtube-thumbs").setHTML(youtube); },500);
                }
