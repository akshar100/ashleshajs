YUI().add('ashlesha-places', function(Y) {
    ...
    Y.PlaceView = Y.Base.create("PlaceView", Y.PostView, [], {
        ...
        loadPlace: function() {
            var place = this.get("model").get("place"),
                c = this.get("container").one(".place");


            Y.use('yui3-gmaps', function(Y) {
                var gmap = new Y.Gmap({
                    container: c
                });
                gmap.loadMap(place);
    ...
    requires: ['base', 'ashlesha-form', 'ashlesha-common-model', 'ashlesha-api']
});