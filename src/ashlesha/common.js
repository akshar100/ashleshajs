...
YUI().add('ashlesha-common', function(Y) {
	...
	Y.DateField = Y.Base.create("DateField",Y.FormItem,[],{
		...
		addDays:function(month,year){
			var ndays = this.get('month_days'), days = 31, i ,c = this.get('container').one(".dd"),n = Y.Node.create('<option value="">--DD--</option>');
			c.setHTML(n);
			if(year && month)
			{
		...
		addMonths:function(){
			var c = this.get('container').one(".mm"), n = Y.Node.create('<option value="">--MM--</option>'),i,
				months=this.get('month_names');
		...
		addYears:function(start,end){
			var c = this.get('container').one(".yy"), n = Y.Node.create('<option value="">--YY--</option>'),i;
			start = start || 1900;
		...
		monthChanged:function(){
			var c= this.get('container'),m = c.one(".mm"),d=c.one(".dd"),y=c.one('.yy'),year,ndays = this.get("month_names"), selected_day,val;
			val = m.get("value");
			if(val && ndays[val]) //Some month is selected
			{
				...
				if(selected_day){
					...
					{
						this.addDays(val);
						d.one("option").setAttribute("selected","selected");
					}
					else
					{
		...
		setYear:function(y){
			this.get('container').one(".yy").one("option[value="+y+"]");
		},
		setMonth:function(y){
			this.get('container').one(".mm").one("option[value="+y+"]");
		},
		setDay:function(y){
			this.get('container').one(".dd").one("option[value="+y+"]"); 
		}
		...
		ATTRS:{
			value:{
				value:'',
				getter:function(){
					var c = this.get('container'),dd=c.one(".dd").get("value"),mm=c.one(".mm").get("value"),yy=c.one(".yy").get("value");
					if(dd && mm && yy)
					{
						return yy+"-"+mm+"-"+dd;
					}
				...
				setter:function(val)
				{
					var date = val.split("-"), c = this.get('container');
					this.setYear(date[0]);
	...
	Y.LoginView = Y.Base.create("LoginView",Y.AshleshaBaseView,[],{
		...
		onSubmit:function(e){
			var c = this.get('container'),user,password,email,form=e.target.getAttribute("id");
			this.halt(e);
	...
	Y.FormView = Y.Base.create("FormView",Y.AshleshaBaseView,[],{
		...
		onSubmit:function(e){
			var items;
			e.halt();
			item = this.getFormItems();
			
		},
		getFormItems:function(){
			var items = [], nodes = this.get('container').all('.yui3-input-container');
			Y.log(nodes);
	...
	requires:['base','cache',function(){if(typeof document !== 'undefined'){ return 'client-app'; } else { return 'server-app'; }}(),'ashlesha-api']
});