...
api.list = {
	...
	"FanPageList":function(config,callback){
		db.view("pages/fanpages",{limit:10,descending:true},function(err,res){
			var list=[];
			  res.forEach(function (row) {
			  	row.id = row._id;
		        list.push(row);
	...
	"FanPagePosts":function(config,callback){
		db.view("posts/getFanpagePosts",{limit:10,descending:true,key:config.pageID},function(err,res){
			var list=[];
			  res.forEach(function (row) {
			  	row.id = row._id;
		        list.push(row);
	...
	update:function(config,callback){
		...
		db.save('_design/user', {
		 ...
		 db.save('_design/posts', {
		    views: {
		      getPosts: {
		        map: 'function(doc){ if(doc.type=="PostModel"){ emit(doc.created_at,doc); }}'
		      },
		      getFanpagePosts:{
		      	map:'function(doc){ if(doc.type==="PostModel" && doc.tType && doc.tType==="fanpage"){ emit(doc.owner_id,doc); } }'
		      }
		    }
		 },function(err,data){
...
exports.api = function(path,config,callback){
		...
		switch(path){
			...
			case "/list/PostList":
				return api.list.PostList(config,callback);
			case "/list/FanPageList":
				return api.list.FanPageList(config,callback);
			case "FanPagePosts":
				return api.list.FanPagePosts(config,callback);
